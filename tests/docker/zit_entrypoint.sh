...
# 1) If user set HOST_IP env, use it
...
if [ -n "$HOST_UID" ] && [ -n "$HOST_GID" ] && command -v gosu >/dev/null 2>&1; then
    mkdir -p /data /config 2>/dev/null || true
    chown -R "$HOST_UID:$HOST_GID" /data /config 2>/dev/null || true
    exec gosu "$HOST_UID:$HOST_GID" zit "$@"
fi
